...
for (type in c("linear", "exp")){
  ...
  if(type == "exp"){
    typeLabel = "Exponential original parameters"
  }
  else {
    typeLabel = "Linear original parameters"
  }
...
barwidth = 0.9
bargap = (1-barwidth)/2
gBar = grid.arrange(textGrob(""), rectGrob(gp=gpar(col="grey", lty=1, alpha=0.5)), textGrob(""), widths=c(bargap,barwidth,bargap))
